...
Sub proxyoff
     ...
     if Wsh.AppActivate("Microsoft Lync")=false then
          if Wsh.AppActivate("Internet Properties")=false then Exit Sub
     End If
     Dim Count : Count = 0
     Do While Count < 2
          ret = Wsh.AppActivate("Microsoft Lync")
          If ret = True Then
               Delay 2
               Wsh.SendKeys "%{F4}"
          End If
          ret = Wsh.AppActivate("Internet Properties")
          If ret = True Then
               Delay 1
               ...
               Delay 1
               Wsh.SendKeys "~"
               ' MsgBox "App Open and shut"
               Exit Do
          End If
          Count = count + 1
          ' MsgBox "Count = " & CInt(Count)
          Delay 2
     Loop
     Set Count = Nothing
     Set ret = Nothing
...
Function read_select(param)
     ...
     If objFSO.FileExists(trim(prefix) & "-dataTables.txt") = true Then
          if debg then Msgbox trim(prefix) & "-dataTables.txt Exists!"  '  uncomment for debugging
     Else
          Msgbox trim(prefix) & "-dataTables.txt Doesnt Exist!" & vbcrlf _
               & vbcrlf & "Will create one for you with default data."

MsgBox ".vbs read_select line 67 - next line quits"
wscript.quit
     ...
     Do Until objFile.AtEndOfStream
          ...
          if debg then msgbox mec : debg=false
...
Function read_buttons()
		' Debg=true
     if debg then msgbox "Read Buttons"
...
MsgBox ".vbs read_buttone line 111 - next line quits"
wscript.quit
...
Function read_data(param)
     if debg then msgbox param & " is passed to read_data vbs"
     ' Set up Data Table
     Dim objFSO, objFile
     Set objFSO = CreateObject("Scripting.FileSystemObject")
     ' Make sure dataTable.html exists, else create it
     If objFSO.FileExists(prefixdt & param & ".html") = true Then
           if debg then Msgbox prefixdt & param & ".html Exists!" : debg=false
     Else
          Msgbox prefixdt & param & ".html Doesnt Exist!" & vbcrlf _
               & vbcrlf & "Will create one for you with default data."

MsgBox ".vbs read_data line 159 - next line quits"
wscript.quit
     ...
     Do Until objFile.AtEndOfStream
          ...
          If InStr(mec, "<") <>1 then
               mec = mec & "<br>"
               if debg then msgbox "Replaced: " & mec : debg=false
          End If
     ...
     apPGDivDatatable.innerHTML = tryit
...
' 
     ...
     if debg then msgbox CStr(mec.rows.length) : debg=false
...
Sub addButton  '  (shorts, texts)
     ...
     if debg then Msgbox shorts & ": " & texts  '  uncomment to debug
...
Sub AddRow(shorts, texts)
     ...
     if debg then MsgBox int  '  uncomment to debug
...
Sub copytoHTM(x)
     Dim objFSO, objFile
     Set objFSO = CreateObject("Scripting.FileSystemObject")
     if debg then msgbox "copyhtm " & x : debg=false
...
Sub addEdited(datatblSelvalue, whichRow, newOnes, newTwos)
     debg=false : if debg then msgbox datatblSelvalue & " " & whichRow _
          & vbcrlf & newOnes _
...
Sub start_timer(timer)
     ...
     ' msgbox mec
     Set mec = nothing
     ' infoBox("AutoPress Done")  '  call infoBox
End Sub
...
Function pgInfoBox2(dispText, dispTime)
     Dim S : Set S = CreateObject("WScript.Shell")
...
Function pgInfoBox3(dispText, dispTime, dispSize)
     Dim S : Set S = CreateObject("WScript.Shell")
     S.Run """" & trim(prefix) & "-infobox.hta"" """ & dispText _
